...
def get_script_meta_data(soup: BeautifulSoup) -> Dict[str, Any]:
    ...
    if script_tag:
        ...
        logging.info("No Meta found for this webpage.")
        script_content: str = "{}"
    return json.loads(script_content).get("analytics", "")
...
def generate_table_from_html(table_div: Tag) -> Dict[str, Any]:
    """Clean HTML table
...
def get_row_from_HTML(row):
    ...
    if key_node and val_node:
        ...
        key: str = key_node.get_text(strip=True)
        val: str = (
...
def get_ad_description(soup: BeautifulSoup) -> Dict[str, str | List[str]]:
    ...
    if caption_div:
        text: str = caption_div.get_text(strip=True)
    else:
        text = ""
...
def compile_record_json(soup:BeautifulSoup) -> dict[str, Any]:
    ...
    result_data["json_data"] = get_script_json_data(soup=soup)
    result_data["metadata"] = get_script_meta_data(soup=soup)
    result_data["location"] = get_location_meta(soup=soup)
